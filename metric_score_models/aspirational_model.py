"""
Aspirational Scoring Model
Predicts how aspirational a creator appears based on content analysis.

Features used:
- mean_hist_score (positive)
- gemini_genz_word_count (positive) 
- gemini_comment_sentiment_counts.appreciating (positive)
- series_reel_mean (negative)
- avg_captioned_reels (negative)
- mean_scene_score (additional)
- mean_face_density (additional)
- outlier_2sigma_ratio (additional)
"""
...
from aspirational_preprocessor import aspirational_preprocessor
import warnings
...
from config import (
    ...
    ASPIRATIONAL_FEATURES, PERSONALITY_SCORE_RANGES
)

class AspirationalModel:
    """Model for predicting aspirational scores."""
    
    def __init__(self):
        self.model = RandomForestRegressor(**RANDOM_FOREST_PARAMS)
        self.feature_names = ASPIRATIONAL_FEATURES
        self.preprocessor = aspirational_preprocessor
        self.is_fitted = False
        self.score_range = PERSONALITY_SCORE_RANGES['aspirational']
        
    def prepare_features(self, df):
        """Prepare features using the preprocessor."""
        return self.preprocessor.transform(df)
    
    def fit(self, df, target_col='aspirational_score', test_size=None):
        """Train the model with train/validation split."""
        print("🔧 Training aspirational model...")
...
aspirational_model = AspirationalModel()