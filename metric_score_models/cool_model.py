"""
Cool Scoring Model
Predicts how cool/trendy a creator appears based on content analysis.

Features used:
- mean_hist_score (positive)
- gemini_comment_sentiment_counts.appreciating (positive)
- avg_english_pct_non_music (positive)
...
from cool_preprocessor import cool_preprocessor
import warnings
...
from config import (
    ...
    COOL_FEATURES, PERSONALITY_SCORE_RANGES
)

class CoolModel:
    """Model for predicting cool scores."""
    
    def __init__(self):
        self.model = RandomForestRegressor(**RANDOM_FOREST_PARAMS)
        self.feature_names = COOL_FEATURES
        self.preprocessor = cool_preprocessor
        self.is_fitted = False
        self.score_range = PERSONALITY_SCORE_RANGES['cool']
        
    def prepare_features(self, df):
        """Prepare features using the preprocessor."""
        return self.preprocessor.transform(df)
    
    def fit(self, df, target_col='cool_score', test_size=None):
        """Train the model with train/validation split."""
        print("🔧 Training cool model...")
