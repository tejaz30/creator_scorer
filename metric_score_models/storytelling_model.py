"""
Storytelling scoring model using XGBoost.
"""
import pandas as pd
import numpy as np
import joblib
...
from config import (
    ...
    MIN_STORYTELLING_SCORE, MAX_STORYTELLING_SCORE, MODELS_DIR, RESULTS_DIR
)
from storytelling_preprocessor import StorytellingFeaturePreprocessor

class StorytellingScorer:
    """XGBoost-based storytelling scoring model."""
    
    def __init__(self, params=None):
        self.params = params or XGBOOST_PARAMS.copy()
        self.model = None
        self.preprocessor = StorytellingFeaturePreprocessor()
        self.is_fitted = False
        self.feature_importance = None
        self.training_metrics = {}
        
    def prepare_data(self, df, target_column='storytelling_score'):
        """Prepare data for training."""
        print("🔄 Preparing storytelling data...")
        ...
        if target_min < MIN_STORYTELLING_SCORE or target_max > MAX_STORYTELLING_SCORE:
            print(f"   ⚠️  Target values outside expected range [{MIN_STORYTELLING_SCORE}, {MAX_STORYTELLING_SCORE}]: [{target_min}, {target_max}]")
    ...
    def train(self, df, target_column='storytelling_score', test_size=TEST_SIZE):
        """Train the storytelling scoring model."""
        print("🚀 Training storytelling scoring model...")
        ...
        train_pred = np.clip(train_pred, MIN_STORYTELLING_SCORE, MAX_STORYTELLING_SCORE)
        test_pred = np.clip(test_pred, MIN_STORYTELLING_SCORE, MAX_STORYTELLING_SCORE)
    ...
    def predict(self, df):
        """Predict storytelling scores for new data."""
        ...
        # Clip to valid range
        predictions = np.clip(predictions, MIN_STORYTELLING_SCORE, MAX_STORYTELLING_SCORE)
    ...
    def predict_with_confidence(self, df, n_estimators_for_std=None):
        ...
        try:
            ...
            lower_bound = np.clip(predictions - confidence_interval, 
                                MIN_STORYTELLING_SCORE, MAX_STORYTELLING_SCORE)
    ...
    def plot_feature_importance(self, top_n=15, save_path=None):
        ...
        plt.title(f'Top {len(importance_df)} Feature Importance - Storytelling Scoring')
        plt.xlabel('Importance Score')
        plt.tight_layout()
        
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Storytelling feature importance plot saved to {save_path}")
    ...
    def save(self, model_path=None, preprocessor_path=None):
        ...
        if model_path is None:
            model_path = MODELS_DIR / "storytelling_model.pkl"
        if preprocessor_path is None:
            preprocessor_path = MODELS_DIR / "storytelling_preprocessor.pkl"
        ...
        print(f"Storytelling model saved to {model_path}")
        print(f"Storytelling preprocessor saved to {preprocessor_path}")
    ...
    def load(self, model_path=None, preprocessor_path=None):
        ...
        print(f"Storytelling model loaded from {model_path}")
        print(f"Storytelling preprocessor loaded from {preprocessor_path}")
    
    def hyperparameter_tuning(self, df, target_column='storytelling_score', param_grid=None):
        """Perform hyperparameter tuning using GridSearchCV."""
        print("🔧 Starting storytelling hyperparameter tuning...")
        ...
        print(f"Best storytelling parameters: {grid_search.best_params_}")
